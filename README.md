...
[![Codex Skill](https://img.shields.io/badge/Codex-Skill-black.svg)](https://developers.openai.com/codex)
...
**`anydesign` runs in Claude and Codex**, and **its output is universal**.

- The skill started as a [Claude Agent Skill](https://docs.claude.com/en/docs/agents-and-tools/agent-skills/overview), and this fork also includes Codex-compatible skill metadata/instructions (`SKILL.md` + `agents/openai.yaml`).
- The output is plain **Markdown + W3C DTCG JSON** — feed it to any AI builder (Codex, v0, Lovable, Cursor, Bolt, Claude Code, Windsurf), hand it to a designer, or paste it into a brief. No lock-in.
...
```bash
git clone https://github.com/eleven-it/anydesign.git
```

### 2a. Copy the skill folder into your Claude skills directory
...
### 2b. Or copy the skill folder into your Codex skills directory

```bash
# For personal Codex skills
mkdir -p ~/.codex/skills
cp -r anydesign ~/.codex/skills/

# Then start a new Codex session so the skill registry is refreshed.
```

Codex reads the same `SKILL.md` workflow and may also use `agents/openai.yaml` for UI metadata. The Python scripts remain standalone.
...
The skill activates automatically when Claude or Codex detects design-analysis intent in your
...
load only when the workflow reaches the corresponding step. This keeps the agent context
window efficient.
